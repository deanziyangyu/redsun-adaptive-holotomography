...
def test_all_profiles_use_redsun_configuration_shape() -> None:
    for name, profile in PROFILES.items():
        metadata = profile_metadata(profile)
        assert profile["schema_version"] == 1.0
        assert profile["frontend"] in {"headless", "pyqt", "pyside"}
        assert metadata["profile"] == name
        assert isinstance(metadata["constructs_hardware"], bool)
...
def test_build_simulation_is_inspectable_and_hardware_free(tmp_path: Path) -> None:
    ...
    assert not profile_metadata(application.profile)["constructs_hardware"]
...
def test_dual_detector_factory_is_hardware_free_and_inspectable() -> None:
    ...
    metadata = profile_metadata(PROFILES["detector-simulate"])
    assert metadata["devices"] == [
        {"id": "fluorescence", "backend": "mock"},
        {"id": "dhm", "backend": "mock"},
    ]
    assert not metadata["constructs_hardware"]
